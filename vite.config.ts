import { defineConfig, resolvePackageData, type Plugin } from "vite";
    ...
    server: {
      proxy: {
        "/f1tv": {
          ...
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/f1tv/, ""),
          configure: (proxy) => {
        ...
        "/bitmovin-license": {
          target: "https://licensing.bitmovin.com",
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/bitmovin-license/, ""),
